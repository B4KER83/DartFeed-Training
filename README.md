...
- Rear/environment-facing camera requested via `facingMode: { ideal:
  'environment' }`, with `width`/`height` ideal set to 4032x3024 so the
  browser/device is asked for its highest practical resolution and clamps
  down only as far as it actually needs to.
