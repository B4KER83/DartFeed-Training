...
(function (global) {
  ...
  // Downloads as JSON so it can be attached/pasted directly. `extra` (used
  // by the recording feature below) merges in recording start/end times
  // and filename so a clip and its log pair up unambiguously; `stampOverride`
  // lets a paired export share the exact same filename stamp as its clip.
  function exportDebugLog(extra, stampOverride) {
    ...
    a.href = url;
    var stamp = stampOverride || ((status.session ? status.session.session_id : 'no-session') + '_' + Date.now());
    a.download = 'training_capture_debug_log_' + stamp + '.json';
  ...
  function finishRecording() {
    ...
    var blob = new Blob(recordedChunks, { type: recordingMimeType });
    ...
    // Same stamp on both filenames makes it obvious which log goes with
    // which clip when several get sent over.
    exportDebugLog({
