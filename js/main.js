...
(function (global) {
  ...
  function drawOverlay() {
    ...
    drawAiMarker();
    if (global.TC.FramingGuide) global.TC.FramingGuide.drawIfLive(overlayCtx, overlay.width, overlay.height);
  ...
  function updateStatusUi() {
    ...
    if (global.TC.FramingGuide && global.TC.FramingGuide.isLive()) {
      var fg = global.TC.FramingGuide;
      var r = fg.getLastResult();
      var pill = $('stat-framing');
      pill.textContent = r ? fg.statusLabel(r.status) : 'MEASURING…';
      pill.style.background = r && r.status === 'PASS' ? '#3ddc6a' : (r ? '#ff5252' : '#999');
      if (r && r.diamFracW != null) {
        $('stat-framing-diam').textContent =
          (r.diamFracW * 100).toFixed(1) + '% w / ' + (r.diamFracH * 100).toFixed(1) + '% h';
        $('stat-framing-centre').textContent =
          (r.centerFracX * 100).toFixed(1) + '%, ' + (r.centerFracY * 100).toFixed(1) + '%';
      } else {
        $('stat-framing-diam').textContent = '—';
        $('stat-framing-centre').textContent = '—';
      }
  ...
  function initCaptureTab() {
    ...
    $('btn-start-session').addEventListener('click', function () {
      ...
      global.TC.Camera.startCamera().then(function () {
        ...
        $('btn-save-empty-board').disabled = false;
        $('btn-framing-guide').disabled = false;
        $('btn-record').disabled = false;
        $('record-unsupported').classList.add('hidden');
        $('stat-empty-saved').textContent = '0';
      }).catch(function (err) {
    ...
    $('btn-end-session').addEventListener('click', function () {
      ...
      teardownZoomControl();
      if (global.TC.FramingGuide) global.TC.FramingGuide.stopLive();
      $('btn-framing-guide').textContent = 'Framing Guide';
      $('framing-guide-panel').classList.add('hidden');
      ...
      $('btn-save-empty-board').disabled = true;
      $('btn-framing-guide').disabled = true;
      $('btn-record').disabled = true;
      updateStatusUi();
    });

    // Temporary data-collection aid (see js/framingGuide.js) — shows a live
    // target circle + PASS/TOO CLOSE/TOO FAR/OFF-CENTRE readout so a new
    // empty-board negative can be framed to match the positive dataset
    // before it's saved. Purely visual; the actual save-time gate in
    // stateMachine.saveEmptyBoardBaseline() re-measures the real captured
    // frame independently of whether this is toggled on.
    $('btn-framing-guide').addEventListener('click', function () {
      var fg = global.TC.FramingGuide;
      if (!fg) return;
      if (fg.isLive()) {
        fg.stopLive();
        this.textContent = 'Framing Guide';
        this.classList.remove('primary');
        $('framing-guide-panel').classList.add('hidden');
      } else {
        fg.startLive(video);
        this.textContent = 'Stop Framing Guide';
        this.classList.add('primary');
        $('framing-guide-panel').classList.remove('hidden');
      }
    });
    ...
    $('btn-save-empty-board').addEventListener('click', function () {
      var btn = this;
      btn.disabled = true;
      sm.saveEmptyBoardBaseline().then(function () {
        var status = sm.getStatus();
        return global.TC.Storage.getNegativesForSession(status.session.session_id);
      }).then(function (negatives) {
        $('stat-empty-saved').textContent = negatives.length;
        btn.disabled = false;
      }).catch(function (err) {
        alert('Could not save empty board: ' + err.message);
        btn.disabled = false;
      });
    });
