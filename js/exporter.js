...
(function (global) {
  ...
  function metadataFor(capture, round) {
    return {
      ...
      ai_tip_x: capture.ai_tip_x != null ? capture.ai_tip_x : null,
      ai_tip_y: capture.ai_tip_y != null ? capture.ai_tip_y : null,
  ...
  function metadataForNegative(n) {
    return {
      negative_id: n.negative_id,
      session_id: n.session_id,
      round_id: n.round_id,
      label: n.label,           // 'negative'
      category: n.category,     // 'empty_board'
      captured_at: n.captured_at,
      saved_at: n.saved_at,
      image_width: n.image_width,
      image_height: n.image_height,
      detection_method: n.detection_method,
      annotation_status: n.annotation_status, // 'not_applicable' — never a dart-tip annotation
      dart_tip_annotation: null,
      framing_check: n.framing_check || null // see js/framingGuide.js — geometry measured at save time
    };
  }
  ...
  function exportSession(sessionId, onProgress) {
    return ensureJsZip().then(function () {
      return Promise.all([
        ...
        global.TC.Storage.getNegativesForSession(sessionId)
      ]);
    }).then(function (results) {
      var session = results[0], rounds = results[1], captures = results[2], negatives = results[3];
      ...
      // Negatives live in their own top-level folder, entirely separate
      // from round_NNNN/ — existing export format for positive captures is
      // untouched. No dart-tip annotation is ever attached to these.
      if (negatives.length > 0) {
        var negFolder = root.folder('negatives');
        var negMetaAll = { label: 'negative', category: 'empty_board', images: [] };
        negatives.sort(function (a, b) { return (a.captured_at || '').localeCompare(b.captured_at || ''); }).forEach(function (n, i) {
          var filename = 'empty_board_' + global.TC.Utils.pad4(i + 1) + '.jpg';
          negFolder.file(filename, n.image_blob);
          var meta = metadataForNegative(n);
          meta.filename = filename;
          negMetaAll.images.push(meta);
        });
        negFolder.file('metadata.json', JSON.stringify(negMetaAll, null, 2));
      }
