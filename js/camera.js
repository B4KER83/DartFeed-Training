...
(function (global) {
  ...
  // Draws the CURRENT video frame into a canvas synchronously. Never
  // upscaled. Downscaled only if it exceeds MAX_CAPTURE_LONG_EDGE, aspect
  // ratio always preserved — this is enforced here in software rather
  // than trusted to getUserMedia's constraints, because "ideal" width/
  // height/aspectRatio are only hints: iOS Safari in particular commonly
  // ignores a requested portrait shape and delivers its native ~4032x3024
  // sensor frame regardless of what was asked for.
  //
  // Confirmed on a real 2026-09-21 capture session: the delivered frame
  // came back 2560x1920 LANDSCAPE (exactly the native ~4032x3024 sensor
  // frame, downscaled) while this app is always used portrait (phone
  // mounted over the board) and every reference photo (both existing
  // positive datasets) is portrait. Previously this function only
  // downscaled whatever shape it was handed, never re-oriented it — so
  // the saved image kept the wrong orientation outright, and on several
  // frames the board's diameter measured LARGER than the frame height
  // (the board didn't even fully fit top-to-bottom). This app has no
  // legitimate landscape use, so a landscape-shaped delivered frame is
  // now rotated back to portrait in software before it's downscaled or
  // saved. CAPTURE_ROTATE_LANDSCAPE_DEG (Config tab, default 90) picks
  // the rotation direction — if a saved image comes out upside-down or
  // mirrored the wrong way for your actual mounting, flip it to -90
  // there rather than needing a code change.
  //
  // Both the master image and its thumbnail must be derived from this one
  // snapshot rather than re-reading the live <video> element later, since
  // the video keeps playing while the (async) toBlob encoding happens —
  // re-sampling it a second time after that gap could pick up a newer
  // frame (e.g. the next dart already landing on a fast throw).
  function captureFrameSnapshot() {
    ...
    var cfg = (global.TC && global.TC.Config && global.TC.Config.current) || {};
    var maxLongEdge = cfg.MAX_CAPTURE_LONG_EDGE || 2560;

    var needsRotate = nativeW > nativeH; // landscape delivered, portrait always intended
    var rotateDeg = needsRotate ? (cfg.CAPTURE_ROTATE_LANDSCAPE_DEG || 90) : 0;
    var orientedW = needsRotate ? nativeH : nativeW;
    var orientedH = needsRotate ? nativeW : nativeH;

    var longEdge = Math.max(orientedW, orientedH);
    var scale = longEdge > maxLongEdge ? (maxLongEdge / longEdge) : 1;
    var w = Math.round(orientedW * scale), h = Math.round(orientedH * scale);
    ...
    if (needsRotate) {
      // Rotate about the canvas centre, drawing the native (unrotated)
      // video frame at native-size * scale so its rotated bounding box
      // exactly fills the w x h (already-swapped) canvas.
      ctx.save();
      ctx.translate(w / 2, h / 2);
      ctx.rotate(rotateDeg * Math.PI / 180);
      ctx.drawImage(videoEl, -(nativeW * scale) / 2, -(nativeH * scale) / 2, nativeW * scale, nativeH * scale);
      ctx.restore();
    } else {
