...
(function (global) {
  ...
  // sensor frame regardless of what was asked for. Both the master image
  // and its thumbnail must be derived from this one snapshot rather than
  // re-reading the live <video> element later, since the video keeps
  ...
  function captureFrameSnapshot() {
    ...
    var maxLongEdge = (global.TC && global.TC.Config && global.TC.Config.current.MAX_CAPTURE_LONG_EDGE) || 2560;
    var longEdge = Math.max(nativeW, nativeH);
    var scale = longEdge > maxLongEdge ? (maxLongEdge / longEdge) : 1;
    var w = Math.round(nativeW * scale), h = Math.round(nativeH * scale);
