...
(function (global) {
  ...
  function StateMachine() {
    ...
    // Full-resolution, clean (no HUD/overlay) camera frame captured at the
    // exact moment the empty-board baseline was (re-)established — the
    // source image for the "Save Empty Board" control. Refreshed every
    // time the baseline itself refreshes; never touched by detection math.
    this._emptyBaselineSnapshot = null;
  ...
  StateMachine.prototype._runEmptyDetecting = function (gray, frameStable, cfg) {
    ...
    if (this._emptyStableTicks >= cfg.EMPTY_BASELINE_STABLE_FRAMES) {
      ...
      this._lastConfirmedAreaVsEmpty = 0;
      this._emptyStableTicks = 0;
      this._retainEmptyBaselineSnapshot();
      this._startRound();
    }
  };

  // Grabs the actual full-resolution camera frame (same source/path as any
  // ordinary dart capture — straight from the <video> element, no HUD or
  // overlay drawn on it) at the moment the empty-board baseline is
  // established or refreshed, so "Save Empty Board" always has a genuinely
  // current clean frame to save rather than reusing a stale one from
  // earlier in the session. Best-effort: if the camera isn't ready for any
  // reason, the previous snapshot (if any) is simply left in place.
  StateMachine.prototype._retainEmptyBaselineSnapshot = function () {
    try {
      var snap = global.TC.Camera.captureFrameSnapshot();
      this._emptyBaselineSnapshot = {
        canvas: snap.canvas,
        width: snap.width,
        height: snap.height,
      ...
      this._log('could not retain empty-board snapshot: ' + err.message);
    }
  };
  ...
  // True only while the board is CURRENTLY in its confirmed/stable-empty
  // resting state (READY_FOR_DART_1) — i.e. the background-diff baseline
  // logic (_runEmptyDetecting / _confirmBoardEmpty), not the AI detector,
  // currently believes zero darts are on the board. This is the sole
  // resting state reached after either path confirms empty: initial
  // baseline establishment and post-removal re-confirmation both end by
  // calling _startRound(), which sets exactly this state. Deliberately
  // NOT based on _emptyBaselineSnapshot merely existing (that only proves
  // a baseline was established at SOME point this session, not that the
  // board is empty right now) and deliberately NOT based on AI confidence
  // (v0.3 has confirmed real-world false-positive "dart present" readings
  // on a genuinely empty board, so its confidence score cannot be trusted
  // as a presence/absence signal — see the false-positive investigation).
  StateMachine.prototype.hasEmptyBaselineSnapshot = function () {
    return !!this._emptyBaselineSnapshot && this.state === STATE.READY_FOR_DART(1);
  };

  // Saves a fresh clean empty-board frame as a labelled negative training
  // example. Deliberately separate from _captureDart: no round/dart_number,
  // no dart-tip fields, no annotation of any kind — this must never be
  // mistaken for a positive (dart) capture by any downstream tool. Same
  // image resolution/JPEG quality as an ordinary capture
  // (cfg.CAPTURE_JPEG_QUALITY, same MAX_CAPTURE_LONG_EDGE cap applied by
  // Camera.captureFrameSnapshot()).
  //
  // Grabs the live frame at the moment of the click (same synchronous
  // Camera.captureFrameSnapshot() call _captureDart uses), rather than
  // re-saving the cached _emptyBaselineSnapshot from whenever the baseline
  // was last (re-)established. Reusing that cached canvas was the bug
  // behind the 2026-09-21 export sessions: every "Save Empty Board" press
  // within a session re-encoded the same single frame, so 10 presses
  // produced 10 byte-identical images instead of 10 distinct stills.
  // Gated on hasEmptyBaselineSnapshot(), i.e. the board must be in its
  // CURRENT confirmed/stable-empty resting state (STATE.READY_FOR_DART_1)
  // right now, at the moment of the click — not merely "was confirmed
  // empty at some earlier point this session". This is background-diff
  // state, never the AI detector's confidence score (see note above on
  // why v0.3 confidence cannot be trusted as a presence/absence gate).
  StateMachine.prototype.saveEmptyBoardBaseline = function () {
    if (!Utils) Utils = global.TC.Utils;
    if (!this.session) return Promise.reject(new Error('No active session — start a session first.'));
    if (!this.hasEmptyBaselineSnapshot()) return Promise.reject(new Error('Board is not currently confirmed empty — wait until the board settles back to its empty resting state, then try again.'));
    ...
    var snap;
    try {
      snap = Camera.captureFrameSnapshot(); // synchronous — captures THIS instant's frame, not the cached baseline
    } catch (err) {
      return Promise.reject(err);
    }

    // Framing gate (see js/framingGuide.js) — the 2026-09-21 negative batch
    // was captured at a much wider field of view than the 61-image
    // positive dataset (board ~75%/~100% of frame width/height vs. the
    // positives' ~93%/~70%), which a model could use as a trivial
    // shortcut instead of learning dart presence. This re-measures the
    // ACTUAL frame being saved — independent of whether the live Framing
    // Guide overlay is toggled on — and refuses to save outside the
    // positive dataset's own measured tolerance, rather than silently
    // saving another mismatched batch.
    var framing = null;
    if (global.TC.FramingGuide) {
      framing = global.TC.FramingGuide.measure(snap.canvas);
      if (framing.status !== 'PASS') {
        var why = framing.reasons.join(' ') || ('Framing check: ' + framing.status);
        return Promise.reject(new Error(
          'Framing check failed (' + global.TC.FramingGuide.statusLabel(framing.status) + '): ' + why +
          (framing.diamFracW != null ? ' [measured ' + (framing.diamFracW * 100).toFixed(1) + '% w / ' +
            (framing.diamFracH * 100).toFixed(1) + '% h, target ' +
            (global.TC.FramingGuide.TARGET_DIAM_FRAC_W * 100).toFixed(0) + '% / ' +
            (global.TC.FramingGuide.TARGET_DIAM_FRAC_H * 100).toFixed(0) + '%]' : '')
        ));
      }
    }

    var capturedAt = new Date().toISOString();
    var imageBlobPromise = new Promise(function (resolve, reject) {
      snap.canvas.toBlob(function (blob) {
        if (!blob) { reject(new Error('toBlob failed')); return; }
        resolve(blob);
      }, 'image/jpeg', cfg.CAPTURE_JPEG_QUALITY);
    });
    var thumbBlobPromise = Camera.makeThumbnailFromSnapshot(snap.canvas, snap.width, snap.height, cfg.THUMBNAIL_WIDTH, 0.8);

    return Promise.all([imageBlobPromise, thumbBlobPromise]).then(function (results) {
      var record = {
        negative_id: self.session.session_id + '_empty_' + Utils.randomHex(6),
        session_id: self.session.session_id,
        round_id: self.round ? self.round.round_id : null, // provenance only — not a dart capture
        label: 'negative',
        ...
        image_width: snap.width,
        image_height: snap.height,
        detection_method: 'empty_baseline_manual_save',
        annotation_status: 'not_applicable',
        dart_tip_annotation: null, // explicitly never annotated — this is a negative example
        framing_check: framing ? {
          status: framing.status,
          diam_frac_w: Number(framing.diamFracW.toFixed(4)),
          diam_frac_h: Number(framing.diamFracH.toFixed(4)),
          center_frac_x: Number(framing.centerFracX.toFixed(4)),
          center_frac_y: Number(framing.centerFracY.toFixed(4)),
          target_diam_frac_w: global.TC.FramingGuide.TARGET_DIAM_FRAC_W,
          target_diam_frac_h: global.TC.FramingGuide.TARGET_DIAM_FRAC_H
        } : null,
        image_blob: results[0],
        thumbnail_blob: results[1]
      };
      return global.TC.Storage.saveNegative(record).then(function () {
        self._log('empty-board frame saved as negative example (' + record.negative_id + ')');
        return record;
      });
    });
  };
