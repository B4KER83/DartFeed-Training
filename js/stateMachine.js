...
(function (global) {
  ...
  // Saves the currently-retained clean empty-board frame as a labelled
  // negative training example. Deliberately separate from _captureDart:
  // no round/dart_number, no dart-tip fields, no annotation of any kind —
  // this must never be mistaken for a positive (dart) capture by any
  // downstream tool. Same image resolution/JPEG quality as an ordinary
  // capture (cfg.CAPTURE_JPEG_QUALITY, same MAX_CAPTURE_LONG_EDGE cap
  // already applied when the snapshot was taken).
  StateMachine.prototype.saveEmptyBoardBaseline = function () {
    ...
    var snapshot = this._emptyBaselineSnapshot;
    if (!snapshot) return Promise.reject(new Error('No confirmed empty-board frame yet — wait a moment for the board to be detected as empty, then try again.'));
    ...
    var imageBlobPromise = new Promise(function (resolve, reject) {
      snapshot.canvas.toBlob(function (blob) {
        if (!blob) { reject(new Error('toBlob failed')); return; }
        resolve(blob);
      }, 'image/jpeg', cfg.CAPTURE_JPEG_QUALITY);
    });
    var thumbBlobPromise = Camera.makeThumbnailFromSnapshot(snapshot.canvas, snapshot.width, snapshot.height, cfg.THUMBNAIL_WIDTH, 0.8);
