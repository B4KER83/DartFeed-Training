...
(function (global) {
  ...
  StateMachine.prototype.hasEmptyBaselineSnapshot = function () {
    return !!this._emptyBaselineSnapshot;
  };
  ...
  // this._emptyBaselineSnapshot is still used as a gate — it tells us the
  // state machine currently believes the board is empty — but the pixels
  // saved always come from a fresh capture.
  StateMachine.prototype.saveEmptyBoardBaseline = function () {
    ...
    if (!this._emptyBaselineSnapshot) return Promise.reject(new Error('No confirmed empty-board frame yet — wait a moment for the board to be detected as empty, then try again.'));
