...
(function (global) {
  ...
  // v2 adds the 'negatives' store (empty-board baseline saves) alongside
  // the existing sessions/rounds/captures stores — additive only, the
  // onupgradeneeded guards below leave every existing store untouched.
  var DB_VERSION = 2;
  var dbPromise = null;
  ...
  function open() {
    ...
    dbPromise = new Promise(function (resolve, reject) {
      ...
      req.onupgradeneeded = function (e) {
        ...
        if (!db.objectStoreNames.contains('negatives')) {
          var negatives = db.createObjectStore('negatives', { keyPath: 'negative_id' });
          negatives.createIndex('by_session', 'session_id', { unique: false });
        }
      };
  ...
  function deleteSessionCascade(sessionId) {
    return getAllByIndex('rounds', 'by_session', sessionId).then(function (rounds) {
      ...
      return getAllByIndex('negatives', 'by_session', sessionId);
    }).then(function (negatives) {
      return Promise.all(negatives.map(function (n) { return remove('negatives', n.negative_id); }));
    }).then(function () {
  ...
  global.TC.Storage = {
    ...
    // Negatives (empty-board baseline saves) — separate store from
    // 'captures' so a negative can never be mistaken for a dart capture by
    // any code that iterates rounds/captures.
    saveNegative: function (n) { return put('negatives', n); },
    getNegative: function (id) { return get('negatives', id); },
    getNegativesForSession: function (sessionId) { return getAllByIndex('negatives', 'by_session', sessionId); },
    deleteNegative: function (id) { return remove('negatives', id); },
